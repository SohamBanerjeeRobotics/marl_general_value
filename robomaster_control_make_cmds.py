...
for i in range(len(tasks['task_embedding'])):
    train_mapping[tasks['task_string'][i]] = {"embedding": tasks['task_embedding'][i], "goal": tasks['reward_kwargs']['goal'][i]}

eval_mapping = {}
for i in range(len(eval_tasks['task_embedding'])):
    eval_mapping[eval_tasks['task_string'][i]] = {"embedding": eval_tasks['task_embedding'][i], "goal": eval_tasks['reward_kwargs']['goal'][i]}
