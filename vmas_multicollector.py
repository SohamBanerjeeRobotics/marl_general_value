...
def t2j(tensor):
    if isinstance(tensor, list):
        tensor = [t2j(t) for t in tensor]
    return jax.dlpack.from_dlpack(torch.utils.dlpack.to_dlpack(tensor))

def j2t(tensor):
    if isinstance(tensor, list):
        tensor = [j2t(t) for t in tensor]
    return torch.utils.dlpack.from_dlpack(jax.dlpack.to_dlpack(tensor))



def _get_random_action(agent, continuous: bool, env):
    if continuous:
        actions = []
