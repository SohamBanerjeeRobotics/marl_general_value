...
def evaluate_ma_policy(env=None, env_kwargs={}, tasks=None, model_path=None, q_function=None, config=None, eval_split=True, timesteps=50, seed=0, task_indices = None):
    """
    ...
    if task_indices is None:
        ...
        agent_task_idx = jax.random.choice(
            key,
        ...
        # Use specified tasks
        assert len(task_indices) == config['num_agents'], \
            f"Must provide {config['num_agents']} tasks, got {len(task_indices)}"
        agent_task_idx = jnp.array(task_indices)

    agent_tasks = {
        "task_embedding": tasks['task_embedding'][agent_task_idx],
        "reward_function": tasks["reward_function"],
        "done_function": tasks["done_function"],  # FIX: was "done_fuction" (typo)
        "reward_kwargs": {"goal": tasks["reward_kwargs"]["goal"][agent_task_idx]}
    } 

    print(f"✓ Assigned {config['num_agents']} agents with tasks:")
    for i in range(config['num_agents']):
        print(f"  Agent {i}: {tasks['task_string'][agent_task_idx[i]]}")

    # Rollout policy
    print(f"✓ Running {timesteps}-step rollout...")
    data = eqx.filter_jit(rollout_policy)(
        env, q_function, agent_tasks, config['num_agents'], key, timesteps
    )
...
def run_interactive(dt=10):
    ...
    e = MARLEnv(headless=False, scale=512, num_agents=1)  # Larger window
    agent_state = e.reset(jax.random.PRNGKey(0))
    goal = jnp.array([0.0, 0.0])  # Target center of arena
...
if __name__ == '__main__':
    # Example 1: Interactive keyboard control
    # run_interactive()
    
    # Example 2: Evaluate trained policy
    print("=" * 60)
    print("MULTI-AGENT POLICY EVALUATION")
    print("=" * 60)
    
    data, goals, frames, rewards, dones = evaluate_ma_policy(
        env_kwargs={"scale": 512, "headless": True},  # Larger window
        model_path="models/best_model.eqx",  # Change to your trained model path
        timesteps=100,
        seed=42,
        task_indices=[4, 5, 6, 7, 4]
    )
